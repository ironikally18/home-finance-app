...
export default function FixedCostsPage() {
  ...
  const loadAll = async () => {
    ...
    const [
      ...
      supabase
        ...
        .select(
          "id,item_name,wallet_id,account_id,category_id,merchant_name,display_order,is_active"
        )
        .eq("user_id", user.id)
        .eq("is_active", true)
        .order("display_order", {
  ...
  const entryMap = useMemo(() => {
    const map = new Map<string, FixedCostEntry>();

    entries.forEach((entry) => {
      map.set(entry.fixed_cost_item_id, entry);
    });
  ...
  const saveAmount = async (
    ...
    const existing = entryMap.get(item.id);
